...
USER=xxxx    #Set Username here.
PORT=xxxx      #Set port number for web interface (default 8112)
...
case "$1" in
  start)
    ...
    start-stop-daemon --start --chuid $USER --name deluged --pidfile $PIDFileDeluge \
     --background --make-pidfile --exec /usr/local/bin/deluged -- --do-not-daemonize 
    start-stop-daemon --start --chuid $USER --name deluge-web --pidfile $PIDFileWeb \
     --background --make-pidfile --exec /usr/local/bin/deluge-web -- --port $PORT
  ;;
